...
Board::Board(const int size, const int level, Player& p)
{
  ...
  points_ = make_path(size, level); //points are used to provide solvable maps
  arr_ = set_squares(size, level, p); //arr is 1D vector of our gameboard
}
  //Meth heads
  //get_moves find what Moves are possible_moves

  std::vector<int> Board::make_path(int size, int level)
  {
  ...
  bool Board::move_player(Player& p, char m)
  {
      switch(m)
    {
        case 'w' : // move up
  ...
  bool Board::check_exit(Player& p)
  {
  ...
  std::vector<char> Board::set_squares(int size, int level, Player& p)
  {
    p.set_position(0);
    srand (time(NULL));
